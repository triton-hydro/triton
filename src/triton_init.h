...
// Contains setup/IO methods of Triton::triton<T>.

#ifndef TRITON_TRITON_INIT_H
#define TRITON_TRITON_INIT_H
...
namespace Triton {
  ...
  void triton<T>::read_configuration(std::string cfg_dir, int checkpoint_id)
  {
    ...
    if (checkpoint_id > 0)
    {
			cfg_path = project_dir + "/" + arglist.output_folder + "/" + CFG_DIR + "/config_" + std::to_string(checkpoint_id) + ".cfg";
    }
  ...
  void triton<T>::read_matrix_files_sequential()
  {
    ...
    if (rank == 0)
    {
      ...
      if (arglist.checkpoint_id > 0 && strcmp(arglist.output_option.c_str(), "SEQ") == 0)
      {
        ...
        std::string temp_num(std::to_string(arglist.checkpoint_id));
        if (arglist.checkpoint_id < 10)
        {
          temp_num = "0" + temp_num;
        }

				std::string filedirH(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/H_" + temp_num + "_00.out");
        ...
        if(arglist.open_boundaries){
          hot_hin.copy_value_into_ghost_cells();
				...
				std::string filedirQX(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/QX_" + temp_num + "_00.out");
        ...
        if(arglist.open_boundaries){
          hot_qxin.copy_value_into_ghost_cells();
				...
				std::string filedirQY(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/QY_" + temp_num + "_00.out");
        ...
        {
					std::string filedirMaxH(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/MH_" + temp_num + "_00.out");
  ...
  void triton<T>::read_matrix_files_parallel()
  {
    ...
    std::string temp_rank(std::to_string(rank));
    if (rank < 10)
    {
      temp_rank = "0" + temp_rank;
    }

    std::string filedir_dem(arglist.dem_filename + "_" + temp_rank + ".dem");
    if (strcmp(arglist.input_format.c_str(), "ASC") == 0)
    {
    ...
    if(!arglist.n_infile.empty())
    {
      std::string filedir_nin(arglist.n_infile + "_" + temp_rank + ".mann");
    ...
    if (arglist.h_infile.size() > 0)
    {
      std::string filedir_hin(arglist.h_infile + "_" + temp_rank + ".out");
    ...
    if (arglist.qx_infile.size() > 0)
    {
      std::string filedir_qxin(arglist.qx_infile + "_" + temp_rank + ".out");
    ...
    if (arglist.qy_infile.size() > 0)
    {
      std::string filedir_qyin(arglist.qy_infile + "_" + temp_rank + ".out");
    ...
    if (arglist.runoff_map.size() > 0)
    {
      std::string filedir_rmap(arglist.runoff_map + "_" + temp_rank + ".rmap");
    ...
    if (arglist.checkpoint_id > 0)
    {
      ...
      if (arglist.checkpoint_id < 10)
      {
        temp_num = "0" + temp_num;
			...
			std::string filedirH(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/H_" + temp_num + "_" + temp_rank + ".out");
      ...
      if(arglist.open_boundaries){
        sub_hot_hin.copy_value_into_ghost_cells();
			...
			std::string filedirQX(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/QX_" + temp_num + "_" + temp_rank + ".out");
      ...
      if(arglist.open_boundaries){
        sub_hot_qxin.copy_value_into_ghost_cells();
			...
			std::string filedirQY(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/QY_" + temp_num + "_" + temp_rank + ".out");
      ...
      if (arglist.max_value_print_option.size() > 0)
      {
				std::string filedirMaxH(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/MH_" + temp_num + "_" + temp_rank + ".out");
  ...
  void triton<T>::process_boundary_condition()
  {
    ...
    std::vector<ExtBC::extBC<T>> extbc(num_extbc);
    std::vector<int> *relative_bc_index = new std::vector<int> [size];
    ...
    host_relative_bc_index = new int[std::max(host_bc_cells_size,1)];
    host_bc_type = new int[std::max(host_bc_cells_size,1)];
    host_bc_start_index = new int[std::max(host_bc_cells_size,1)];
    host_bc_nrows_vars = new int[std::max(host_bc_cells_size,1)];
      ...
      else
      {
        ...
        if(arglist.open_boundaries){
          sub_hot_hin.copy_value_into_ghost_cells();
				...
				std::string filedirU(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/QX_" + temp_num + "_" + temp_rank + ".out");
        sub_hot_qxin.load_from_binary_file(host_dem_original_row, host_dem_original_col, filedirU);
        sub_hot_qxin.add_ghost_cells(GHOST_CELL_PADDING, GHOST_CELL_PADDING, 0.0);
        if(arglist.open_boundaries){
          sub_hot_qxin.copy_value_into_ghost_cells();
        }
				std::string filedirV(project_dir + "/" + arglist.output_folder + "/" + BIN_DIR + "/QY_" + temp_num + "_" + temp_rank + ".out");
  ...
  void triton<T>::create_host_aux_vectors()
  {
	...
	host_obs_h = new T[std::max(num_of_obs_points,1)];
	host_obs_qx = new T[std::max(num_of_obs_points,1)];
	host_obs_qy = new T[std::max(num_of_obs_points,1)];
  }
  ...
  void triton<T>::create_device_vectors()
  {
    ...
    int nbytes_bc_cell_size = (sizeof(int) * std::max(host_bc_cells_size,1));
    int nbytes_bc_vars = (sizeof(T) * host_bc_vars_arr_size);
    int nbytes_obs_cell_size = (sizeof(int) * std::max(host_obs_cells_size,1));
